...
const Management = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      const response = await fetch("http://172.30.1.26:3001/api/users/", {
  ...
  const getPageData = async (page) => {
    ...
    const URL = `http://172.30.1.26:3001/api/users/${page}`;
  ...
  const updateClick = (evt) => {
    ...
    if (globalArray.length > 1) {
      // 회원수정은 동시에 여러개를 할수 없으니, 1개만 가능하도록 validation 처리 했습니다.
      alert("수정은 한개의 아이디만 가능합니다.");
      return;
    }
  ...
  const deleteClick = (evt) => {
    ...
    fetch("http://172.30.1.26:3001/api/user/delete", {
      headers: {
      ...
      .then((result) => {
        if (result.success === true) {
          ...
          alert(`${globalArray.length}개의 회원정보가 삭제되었습니다.`); // 알림창
  ...
  const searchClick = async (evt) => {
    ...
    const response = await fetch(
      `http://172.30.1.26:3001/api/user/search/${searchInput}`,
      {
        method: "GET",
      }
    );
        ...
        <DataTableContent>
          <DataTableHead>
            <DataTableRow>
              ...
              <DataTableHeadCell>회원번호</DataTableHeadCell>
              <DataTableHeadCell>아이디</DataTableHeadCell>
              <DataTableHeadCell>이름</DataTableHeadCell>
              <DataTableHeadCell>이메일</DataTableHeadCell>
              <DataTableHeadCell>계정권한</DataTableHeadCell>
              <DataTableHeadCell>전화번호</DataTableHeadCell>
              <DataTableHeadCell>[회원사진 저장경로]</DataTableHeadCell>
              <DataTableHeadCell>주소</DataTableHeadCell>
              <DataTableHeadCell>나머지주소</DataTableHeadCell>
            </DataTableRow>
          </DataTableHead>
          <DataTableBody>
            {state.map((v, i) => (
